Full "ship it" workflow for .NET: verify, commit, push, PR, link work item, notify.
...
3. **Extract the ADO work item ID** from branch name (`feature/AB1234-...`
   or `feature/1234-...`). If not found, ask me.

4. **Commit**
   - Stage only files I touched. Never `git add -A`.
   - Conventional Commits message from the diff:
     `type(scope): summary (AB#1234)`. Body when the diff warrants.
...
6. **Open the PR via GitHub MCP**
   ...
   - Body: `## Summary` + `## Test plan` + `Fixes AB#1234`. Note breaking changes.
...
7. **Link the work item via Azure DevOps MCP**
   ...
   - `wit_update_work_item` — state from `Active` → `Code Review`.
   - `wit_add_work_item_comment` — one-line: `PR opened: <url>`.
