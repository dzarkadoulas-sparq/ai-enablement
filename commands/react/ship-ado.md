Full "ship it" for React SPA: verify, commit, push, PR, link work item, notify.
...
3. **Visual verification** — if the diff touches any `.tsx` under
   ...
   - Playwright MCP: navigate each affected route, screenshot, check console.
   - Abort if new console errors appear.
...
7. **Open the PR via GitHub MCP**
   ...
   - Body: `## Summary` + `## Test plan` + `## Screenshots` + `Fixes AB#1234`.
...
`--no-verify`; never weaken tests, a11y assertions, or security.
