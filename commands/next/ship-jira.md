Full "ship it" for Next.js: verify, commit, push, PR, link ticket, notify.
...
2. **Gate** — run everything `/pre-pr` runs (lint, typecheck, test,
   build, bundle-size, audit, `/verify-caching` if relevant). Stop on
   any failure.
...
3. **Visual verification** — if the diff touches `app/**`, `components/**`,
   ...
   - Abort on any new console / server error.
...
7. **Open the PR via GitHub MCP**
   ...
   - Body: `## Summary` + `## Test plan` + `## Screenshots` +
     `## Caching impact` (list revalidated tags / routes) + `Closes PROJ-123`.
